...
- `SessionStart + codex-session-watch` 只为 `QuestionNotification` 保留。
- 这里的 `QuestionNotification` 对应 Codex app UI 里的 question notifications。
...
- [`codex-approval.md`](./codex-approval.md)：只放当前仍生效的 Codex hooks、`QuestionNotification` watcher 和 `SessionStart` bootstrap 语义。
